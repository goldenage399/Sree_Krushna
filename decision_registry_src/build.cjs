/**
 * Static Decoupled Component Assembler (SDCA) Compiler — Decision Registry
...
console.log('⚡ Building Decision Registry from modular components (SDCA)...');
...
// Read Body Component and Shared Modals
...
console.log('✅ Assembled standalone decision-registry.html (' + fs.statSync(target1).size + ' bytes)');

// 6. Build Scoped Fragment HTML (INC-086 Monolithic Engine Port Gate)
function scopeSelector(sel, prefix) {
  ...
  if (sel.startsWith('#tab-decision-registry') || sel.startsWith('#decisionRegistryFrame')) return sel;
  return prefix + ' ' + sel;
}
...
const fragmentPrefix = '#tab-decision-registry #decisionRegistryFrame';
const scopedCss = scopeCssBlock(combinedCss, fragmentPrefix);

const fragmentHtml = `<!-- Sree Krushna Marriage OS — Master Decision Registry Scoped Fragment -->
<!-- Standard: STD-MOD-COMP-001 | Ruling: UDS-HIERARCHY-001 -->
<style data-source="decision-registry-fragment">
${scopedCss}
</style>

<div id="decisionRegistryFrame">
${bodyHtml}
</div>
...
const targetFragmentRoot = path.join(rootDir, 'decision-registry-fragment.html');
const targetFragmentPublic = path.join(rootDir, 'public', 'decision-registry-fragment.html');
