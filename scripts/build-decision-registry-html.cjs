/**
 * Static Decoupled Component Assembler (SDCA) for Decision Registry & Visual Comparator
 ...
 * 
 * Generates both decision-registry.html and public/decision-registry.html with 100% byte parity.
 */
...
<head>
  ...
  <title>Sree Krushna Marriage OS — Master Decision Registry & Event Stepper</title>
  <meta name="description" content="Single-window executive dashboard for event-by-event decision locking, visual option comparison, and family consensus across Sree Krushna Marriage OS.">
  <meta name="theme-color" content="#0f1117">
  <link rel="icon" href="./assets/icons/icon-192x192.png" type="image/png">
  <style>
    /* ==========================================================================
       DECISION REGISTRY & EVENT STEPPER SCOPED CSS (INC-086 COMPLIANT)
       All rules are prefixed with #decisionRegistryRoot, #drLightboxModal, or .dr-*
       to ensure zero global cascade bleed when embedded into the main host app.
       ========================================================================== */
    :root {
      --dr-bg: #0f1117;
      --dr-surface: #181b24;
      --dr-surface-elevated: #222634;
      --dr-border: #2d3245;
      --dr-border-focus: #e5a93c;
      --dr-text-primary: #f3f4f6;
      --dr-text-secondary: #9ca3af;
      --dr-text-muted: #6b7280;
      --dr-gold: #e5a93c;
      --dr-gold-glow: rgba(229, 169, 60, 0.15);
      --dr-gold-subtle: rgba(229, 169, 60, 0.08);
      --dr-emerald: #10b981;
      --dr-emerald-glow: rgba(16, 185, 129, 0.15);
      --dr-amber: #f59e0b;
      --dr-amber-glow: rgba(245, 158, 11, 0.15);
      --dr-rose: #f43f5e;
      --dr-rose-glow: rgba(244, 63, 94, 0.15);
      --dr-purple: #a855f7;
      --dr-purple-glow: rgba(168, 85, 247, 0.15);
      --dr-blue: #3b82f6;
      --dr-blue-glow: rgba(59, 130, 246, 0.15);
      --dr-radius-sm: 6px;
      --dr-radius-md: 10px;
      --dr-radius-lg: 16px;
      --dr-shadow: 0 4px 20px rgba(0, 0, 0, 0.35);
      --dr-font-sans: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif;
    }
    ...
    /* Family Review Welcome Banner (when ?mode=family is active) */
    .dr-family-banner {
      display: none;
    ...
    .dr-family-banner.active {
      display: flex;
    }

    .dr-family-banner-left {
      display: flex;
      align-items: center;
      gap: 12px;
    }

    .dr-family-banner-title {
      font-size: 16px;
      font-weight: 700;
      color: #ffffff;
      margin: 0;
    ...
    .dr-logo-monogram {
      ...
      height: 44px;
      border-radius: 50%;
      background: linear-gradient(135deg, #e5a93c 0%, #b88226 100%);
      color: #0f1117;
      display: flex;
      align-items: center;
    ...
    .dr-btn-primary {
      background: linear-gradient(135deg, #e5a93c, #c58a23);
      color: #0f1117;
      border: none;
      font-weight: 700;
    }
    ...
    .dr-btn-primary:hover {
      background: linear-gradient(135deg, #f0b548, #d6982f);
    ...
    /* ==========================================================================
       CHRONOLOGICAL EVENT STEPPER (P-EVENT-STEPPER-001)
       ========================================================================== */
    ...
    .dr-milestone-node:hover {
      border-color: var(--dr-border-focus);
      transform: translateY(-2px);
    }

    .dr-milestone-node.active {
      border-color: var(--dr-gold);
      background: linear-gradient(180deg, rgba(229, 169, 60, 0.12) 0%, var(--dr-surface-elevated) 100%);
      box-shadow: 0 0 15px rgba(229, 169, 60, 0.2);
    }

    .dr-milestone-node.locked-state {
      border-color: var(--dr-emerald);
    }
    ...
    .dr-milestone-node.active .dr-milestone-badge {
      background: var(--dr-gold);
      color: #0f1117;
    }

    .dr-milestone-node.locked-state .dr-milestone-badge {
      background: var(--dr-emerald);
      color: #ffffff;
    }
    ...
    .dr-lock-event-btn:hover {
      box-shadow: 0 0 12px rgba(16, 185, 129, 0.4);
      transform: scale(1.02);
    }

    /* ==========================================================================
       MULTI-OPTION CLUSTER POD (P-OPTION-POD-001)
       ========================================================================== */
    .dr-cluster-pod-container {
      margin-bottom: 28px;
    }
    ...
    .dr-cluster-card::before {
      ...
      right: 0;
      height: 4px;
      background: linear-gradient(90deg, #e5a93c, #a855f7);
    }
    ...
    .dr-pod-option-card:hover {
      border-color: var(--dr-gold);
      transform: translateY(-3px);
      box-shadow: 0 8px 20px rgba(0, 0, 0, 0.4);
    }

    .dr-pod-option-card.selected {
      border-color: var(--dr-gold);
      background: linear-gradient(180deg, rgba(229, 169, 60, 0.1) 0%, var(--dr-surface-elevated) 100%);
    ...
    .dr-pod-option-card:hover .dr-pod-img {
    ...
    .dr-pod-option-card.selected .dr-pod-radio-btn {
      background: var(--dr-gold);
      color: #0f1117;
      border-color: var(--dr-gold);
    }

    /* WhatsApp & Consensus Footer Bar */
    .dr-cluster-footer {
      display: flex;
      justify-content: space-between;
      align-items: center;
    ...
    /* Carousel Section (Single Plates) */
    .dr-carousel-section {
      background: var(--dr-surface);
      border: 1px solid var(--dr-border);
      border-radius: var(--dr-radius-lg);
    ...
    .dr-carousel-section::before {
      ...
      right: 0;
      height: 3px;
      background: linear-gradient(90deg, #e5a93c, #a855f7, #3b82f6);
    }
    ...
    .dr-carousel-title-group {
      display: flex;
      align-items: center;
      gap: 12px;
    }

    .dr-carousel-icon-box {
      width: 36px;
      height: 36px;
      border-radius: var(--dr-radius-sm);
      background: var(--dr-gold-glow);
      color: var(--dr-gold);
      display: flex;
      align-items: center;
      justify-content: center;
    ...
    .dr-carousel-subtitle {
      font-size: 12px;
      color: var(--dr-text-secondary);
      margin: 2px 0 0;
    }

    .dr-carousel-nav-controls {
      display: flex;
      align-items: center;
      gap: 10px;
    }

    .dr-carousel-indicator {
      font-size: 12px;
      color: var(--dr-text-muted);
      font-weight: 600;
      padding: 4px 8px;
    ...
    .dr-carousel-track {
      ...
      scrollbar-color: var(--dr-gold) var(--dr-surface-elevated);
    }

    .dr-carousel-track::-webkit-scrollbar {
      height: 6px;
    }

    .dr-carousel-track::-webkit-scrollbar-track {
      background: var(--dr-surface-elevated);
      border-radius: 3px;
    }

    .dr-carousel-track::-webkit-scrollbar-thumb {
      background: var(--dr-gold);
      border-radius: 3px;
    }
    ...
    .dr-plate-media:hover .dr-plate-img {
    ...
    .dr-plate-status-badge.locked { background: rgba(16, 185, 129, 0.85); color: #ffffff; }
    .dr-plate-status-badge.pending { background: rgba(245, 158, 11, 0.85); color: #000000; }
    .dr-plate-status-badge.shortlisted { background: rgba(168, 85, 247, 0.85); color: #ffffff; }
    .dr-plate-status-badge.evaluating { background: rgba(59, 130, 246, 0.85); color: #ffffff; }

    .dr-plate-event-label {
      color: #ffffff;
      font-size: 11px;
      font-weight: 600;
      display: flex;
    ...
    .dr-plate-btn-action:hover {
      background: var(--dr-border);
      border-color: var(--dr-gold);
      color: var(--dr-gold);
    }

    .dr-plate-btn-action.highlight {
      background: var(--dr-gold-glow);
      border-color: var(--dr-gold);
      color: var(--dr-gold);
    }
    ...
    .dr-pill.active {
      ...
      border-color: var(--dr-gold);
      font-weight: 700;
      box-shadow: 0 0 10px rgba(229, 169, 60, 0.3);
    }
    ...
    .dr-search-input:focus {
      border-color: var(--dr-gold);
      box-shadow: 0 0 0 2px var(--dr-gold-glow);
    }
    ...
    .dr-card.target-highlight {
      border-color: var(--dr-gold) !important;
      box-shadow: 0 0 25px rgba(229, 169, 60, 0.6) !important;
    ...
    .dr-card.decor_pending .dr-card-strip { background: var(--dr-amber); }
    .dr-card.decor_vendor .dr-card-strip { background: var(--dr-rose); }
    .dr-card.idea_incubator .dr-card-strip { background: var(--dr-purple); }
    .dr-card.decor_locked .dr-card-strip { background: var(--dr-emerald); }
    .dr-card.governance .dr-card-strip { background: var(--dr-blue); }
    ...
    .dr-status-pill.pending { background: var(--dr-amber-glow); color: var(--dr-amber); border: 1px solid rgba(245, 158, 11, 0.3); }
    .dr-status-pill.vendor { background: var(--dr-rose-glow); color: var(--dr-rose); border: 1px solid rgba(244, 63, 94, 0.3); }
    .dr-status-pill.incubating { background: var(--dr-purple-glow); color: var(--dr-purple); border: 1px solid rgba(168, 85, 247, 0.3); }
    .dr-status-pill.locked { background: var(--dr-emerald-glow); color: var(--dr-emerald); border: 1px solid rgba(16, 185, 129, 0.3); }
    ...
    .dr-benchmark-note strong {
      color: var(--dr-gold);
    }

    /* Decision Card Visual Preview Attachment */
    .dr-card-media-attachment {
    ...
    .dr-card-media-attachment:hover {
      border-color: var(--dr-gold);
      background: var(--dr-gold-subtle);
    }
    ...
    .dr-card-media-attachment:hover .dr-card-media-action {
      color: var(--dr-gold);
    }

    /* Options & Interactive Actions */
    .dr-options-container {
      margin-top: 14px;
      display: flex;
      flex-direction: column;
      gap: 8px;
    ...
    .dr-option-row.selected .dr-radio-circle {
      border-color: var(--dr-gold);
      background: var(--dr-gold);
    }
    ...
    .dr-option-row.selected .dr-radio-inner {
      display: block;
    }

    /* Vendor Deliverable Banner */
    .dr-vendor-box {
    ...
    /* ==========================================================================
       UNIVERSAL LIGHTBOX & STAGE COMPARATOR MODAL (#drLightboxModal)
       ========================================================================== */
    #drLightboxModal {
      display: none;
      position: fixed;
    ...
    .dr-lightbox-header-left {
      display: flex;
      align-items: center;
      gap: 12px;
    }

    .dr-lightbox-title-wrap h3 {
      margin: 0;
      font-size: 16px;
      font-weight: 700;
      color: #ffffff;
    ...
    .dr-lightbox-title-wrap p {
      margin: 2px 0 0;
      font-size: 12px;
      color: var(--dr-text-secondary);
    }

    .dr-lightbox-header-right {
      display: flex;
      align-items: center;
      gap: 8px;
    }

    .dr-lightbox-toggle-btn {
      background: var(--dr-surface);
      border: 1px solid var(--dr-border);
      color: var(--dr-text-secondary);
      padding: 6px 12px;
    ...
    .dr-lightbox-toggle-btn:hover {
      border-color: var(--dr-gold);
      color: #ffffff;
    }

    .dr-lightbox-toggle-btn.active {
      background: var(--dr-gold);
      color: #0f1117;
      border-color: var(--dr-gold);
      font-weight: 700;
    ...
    .dr-lightbox-close-btn:hover {
      background: var(--dr-rose);
      border-color: var(--dr-rose);
      color: #ffffff;
    }

    /* Lightbox Stage Area */
    .dr-lightbox-stage-container {
      position: relative;
      background: #000000;
      display: flex;
      align-items: center;
    ...
    .dr-lightbox-nav-arrow.prev { left: 16px; }
    .dr-lightbox-nav-arrow.next { right: 16px; }

    /* Comparison Split View Inside Lightbox */
    .dr-lightbox-compare-stage {
      display: none;
      width: 100%;
      padding: 16px;
      box-sizing: border-box;
    ...
    .dr-lightbox-compare-stage.active {
      display: flex;
    }
    ...
    .dr-compare-card.current {
      border-color: var(--dr-gold);
      box-shadow: 0 0 15px rgba(229, 169, 60, 0.25);
    }

    .dr-compare-card-media {
      height: 180px;
      position: relative;
      background: #000000;
    }
    ...
    /* Lightbox Meta Details & Drawer */
    .dr-lightbox-meta-drawer {
      padding: 16px 20px;
      background: var(--dr-surface);
      border-top: 1px solid var(--dr-border);
      overflow-y: auto;
    ...
    .dr-lightbox-thumb:hover {
      opacity: 0.9;
    }

    .dr-lightbox-thumb.active {
      opacity: 1;
      border-color: var(--dr-gold);
      box-shadow: 0 0 8px rgba(229, 169, 60, 0.4);
    }

    .dr-lightbox-thumb img {
      width: 100%;
      height: 100%;
      object-fit: cover;
    }
    ...
    @media print {
      ...
      .dr-btn, .dr-toolbar, .dr-carousel-section, .dr-stepper-container, #drLightboxModal, .dr-family-banner {
        display: none !important;
      }
      .dr-cluster-card, .dr-card {
      ...
      .dr-card-title, .dr-cluster-title-wrap h2 {
        color: #000000 !important;
      }
    }
  </style>
...
<div id="decisionRegistryRoot">
  <!-- Family Consensus Mode Welcome Banner -->
  <div class="dr-family-banner" id="familyWelcomeBanner">
    <div class="dr-family-banner-left">
      <span style="font-size: 28px;">🌺</span>
      <div>
        <h2 class="dr-family-banner-title">Welcome to the Sree Krushna Family Decor Review!</h2>
        <p class="dr-family-banner-sub">Please review the shortlisted Mandap and Sangeet Stage options below, tap your preferred choice, and share your vote with the family.</p>
      </div>
    </div>
    <button class="dr-btn dr-btn-primary" onclick="window.switchViewMode('executive')">
      <span>👑</span> Executive Host View
    </button>
  </div>

  <!-- Executive Header -->
  <header class="dr-header" id="executiveHeader">
    <div class="dr-brand">
      <div class="dr-logo-monogram">SK</div>
      <div class="dr-title-group">
        <h1>Master Decision Registry & Event Stepper</h1>
        <p>Sree Krushna Marriage OS — Consolidated Executive Review Surface (Standard P-DECISION-REG-001)</p>
      </div>
    </div>
    <div class="dr-header-actions">
      <button class="dr-btn dr-btn-whatsapp" id="btnShareFamilyMode" title="Copy Family WhatsApp Link">
        <span>📱</span> Share with Family
      </button>
      <button class="dr-btn" id="btnExportJson" title="Export current decisions state">
        <span>💾</span> Export JSON
      </button>
      <button class="dr-btn" id="btnPrintDossier" title="Print Executive Meeting Dossier">
        <span>🖨️</span> Print Dossier
      </button>
      <a href="./decorator-cockpit.html" class="dr-btn dr-btn-primary" title="Open Decorator Negotiation Cockpit">
        <span>🏛️</span> Open Cockpit
      </a>
    </div>
  </header>

  <!-- ==========================================================================
       CHRONOLOGICAL EVENT STEPPER COMPONENT (P-EVENT-STEPPER-001)
       ========================================================================== -->
  <section class="dr-stepper-container" id="eventStepperSection">
    <div class="dr-stepper-header">
      <div class="dr-stepper-title-wrap">
        <span style="font-size: 20px;">🧭</span>
        <div>
          <h2 class="dr-stepper-title">Chronological Event Decision Flow</h2>
          <p class="dr-stepper-desc">Walk through the wedding timeline step-by-step. Select an event milestone to review its decor clusters and lock decisions.</p>
        </div>
      </div>
      <div class="dr-stepper-nav-controls">
        <button class="dr-btn dr-btn-sm" id="btnStepperPrev">‹ Previous Event</button>
        <button class="dr-btn dr-btn-sm" id="btnStepperNext">Next Event ›</button>
      </div>
    </div>

    <!-- Milestone Track (5 Event Chapters) -->
    <div class="dr-milestone-track" id="milestoneTrack">
      <!-- Injected dynamically by JavaScript -->
    </div>

    <!-- Active Event Action Bar -->
    <div class="dr-event-action-bar" id="eventActionBar">
      <div class="dr-event-action-info">
        <span id="activeEventIcon">🪔</span>
        <span id="activeEventSummary">Currently Viewing: <strong>Day 2 Vedic Vivaha</strong> (Muhurtham 10:15)</span>
      </div>
      <button class="dr-lock-event-btn" id="btnLockActiveEvent">
  ...
  <!-- ==========================================================================
       MULTI-OPTION CLUSTER PODS (P-OPTION-POD-001)
       ========================================================================== -->
  <section class="dr-cluster-pod-container" id="clusterPodsContainer">
    <!-- Clustered side-by-side option pods injected dynamically -->
  </section>

  <!-- ==========================================================================
       VISUAL CONCEPT LOOKBOOK CAROUSEL (Individual Plates)
       ========================================================================== -->
  <section class="dr-carousel-section" id="visualShowcaseSection">
    <div class="dr-carousel-header">
      <div class="dr-carousel-title-group">
        <div class="dr-carousel-icon-box">📸</div>
        <div>
          <h2 class="dr-carousel-title">Individual Visual Plate Lookbook</h2>
          <p class="dr-carousel-subtitle">All 12 registered scenic fabrication plates from the Marquee Decor Tender (SSOT: assets/decor/registry.json).</p>
        </div>
      </div>
      <div class="dr-carousel-nav-controls">
        <span class="dr-carousel-indicator" id="carouselCounterIndicator">Showing 12 Plates</span>
        <button class="dr-carousel-arrow" id="btnCarouselPrev" title="Previous plate">‹</button>
        <button class="dr-carousel-arrow" id="btnCarouselNext" title="Next plate">›</button>
      </div>
    </div>

    <!-- Scroll-Snap Horizontal Track -->
    <div class="dr-carousel-track" id="carouselTrack">
      <!-- Injected dynamically by JavaScript -->
    </div>
  </section>

  <!-- Filter & Search Toolbar for Decisions Grid -->
  <div class="dr-toolbar">
    <div class="dr-filter-pills" id="filterPillsContainer">
      <button class="dr-pill active" data-filter="all_pending">
        All Actionable <span class="dr-pill-count" id="countPillActionable">17</span>
      </button>
      <button class="dr-pill" data-filter="decor_pending">
        Decor Choices <span class="dr-pill-count" id="countPillDecor">6</span>
      </button>
      <button class="dr-pill" data-filter="decor_vendor">
        Vendor Deliverables <span class="dr-pill-count" id="countPillVendor">4</span>
      </button>
      <button class="dr-pill" data-filter="idea_incubator">
        💡 Idea Incubator <span class="dr-pill-count" id="countPillIdeas">7</span>
      </button>
      <button class="dr-pill" data-filter="decor_locked">
        🔒 Certified Locked <span class="dr-pill-count" id="countPillLocked">10</span>
      </button>
      <button class="dr-pill" data-filter="all">
        Full Registry <span class="dr-pill-count" id="countPillAll">28</span>
      </button>
    </div>
    <div class="dr-search-box">
      <span class="dr-search-icon">🔍</span>
      <input type="text" id="searchInput" class="dr-search-input" placeholder="Search decisions, specs, keywords...">
    </div>
  </div>

  <!-- Decisions & Ideas Grid -->
  <main class="dr-grid" id="decisionsGrid">
    <!-- Rendered dynamically by JavaScript from DECISION_REGISTRY_DATA -->
  </main>
</div>
...
<!-- ==========================================================================
     UNIVERSAL LIGHTBOX & COMPARISON MODAL (#drLightboxModal)
     ========================================================================== -->
<div id="drLightboxModal" role="dialog" aria-modal="true">
  <div class="dr-lightbox-dialog">
    <!-- Modal Header -->
    <div class="dr-lightbox-header">
      <div class="dr-lightbox-header-left">
        <span class="dr-id-badge" id="modalPlateId">PLATE-01</span>
        <div class="dr-lightbox-title-wrap">
          <h3 id="modalPlateTitle">Vedic Lotus Mandap & Elevated Altar</h3>
          <p id="modalPlateSub">Zone A — Sacred Sanctuary • SPEC-PROC-DECOR-MARQUEE-001</p>
        </div>
      </div>
      <div class="dr-lightbox-header-right">
        <div id="modalBlueprintToggleWrap" style="display: none;">
          <button class="dr-lightbox-toggle-btn active" id="btnViewPhoto">📷 Photo</button>
          <button class="dr-lightbox-toggle-btn" id="btnViewCad">📐 CAD Vector</button>
        </div>
        <button class="dr-lightbox-toggle-btn" id="btnToggleCompare" style="display: none;">⚖️ Compare Alternatives</button>
        <button class="dr-lightbox-close-btn" id="btnCloseLightbox" title="Close (Esc)">✕</button>
      </div>
    </div>

    <!-- Stage Viewport (Single Asset View) -->
    <div class="dr-lightbox-stage-container" id="modalSingleStage">
      <button class="dr-lightbox-nav-arrow prev" id="btnModalPrev">‹</button>
      <img id="modalImg" class="dr-lightbox-view-asset" src="" alt="Decor Inspiration Plate">
      <object id="modalSvgObject" class="dr-lightbox-view-asset" type="image/svg+xml" style="display: none; width: 90%; height: 50vh;"></object>
      <button class="dr-lightbox-nav-arrow next" id="btnModalNext">›</button>
    </div>

    <!-- Comparison Viewport (Multi-Option View) -->
    <div class="dr-lightbox-compare-stage" id="modalCompareStage">
      <!-- Dynamically filled with Option Cards -->
    </div>

    <!-- Meta Details & Specs Drawer -->
    <div class="dr-lightbox-meta-drawer">
      <div class="dr-lightbox-meta-grid">
        <div class="dr-meta-item">
          <strong>Dimensions & Elevation</strong>
          <span id="modalDimensions">—</span>
        </div>
        <div class="dr-meta-item">
          <strong>Event & Zone</strong>
          <span id="modalEventZone">—</span>
        </div>
        <div class="dr-meta-item">
          <strong>Contract Rider Clause</strong>
          <span id="modalClause">—</span>
        </div>
        <div class="dr-meta-item">
          <strong>Linked Decisions</strong>
          <span id="modalLinkedDecisions">—</span>
        </div>
      </div>
      <div class="dr-lightbox-notes-box" id="modalNotesBox">
        <!-- Notes text -->
      </div>
    </div>

    <!-- Bottom Thumbnail Hop Bar -->
    <div class="dr-lightbox-thumbs-bar" id="modalThumbsBar">
      <!-- 12 thumbnails injected dynamically -->
    </div>
  </div>
</div>

<!-- Load Unified Data Layer -->
<script src="./js/decision-registry-data.js"></script>
          ...
          <div class="dr-cluster-card" id="cluster-\${cluster.id}">
            ...
            <div class="dr-pod-grid">
              \${cluster.options.map(opt => {
                ...
                return \`
                  <div class="dr-pod-option-card \${isSelected ? 'selected' : ''}" onclick="window.selectClusterOption('\${cluster.decisionId}', '\${opt.optionId}', '\${cluster.id}')">
                    ...
                    <div class="dr-pod-body">
                      ...
                      <button class="dr-pod-radio-btn" type="button">
                        <span>\${isSelected ? '🔘' : '⚪'}</span>
                        <span>\${isSelected ? 'Chosen Concept (Active)' : 'Select This Concept'}</span>
                        \${voteCount > 0 ? \`<span style="margin-left: auto; color: var(--dr-purple);">(\${voteCount} votes)</span>\` : ''}
                      </button>
                    </div>
                  </div>
                \`;
...
// Write to root and public targets
const target1 = path.join(rootDir, 'decision-registry.html');
const target2 = path.join(rootDir, 'public/decision-registry.html');

fs.writeFileSync(target1, htmlContent, 'utf8');
fs.writeFileSync(target2, htmlContent, 'utf8');

console.log('✅ Successfully wrote updated Decision Registry HTML with Event Stepper & WhatsApp Sharing to:');
